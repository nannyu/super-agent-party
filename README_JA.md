...
- ショートカットコマンド：チャット入力欄、または接続済みの任意の IM プラットフォーム（Feishu、QQ、Telegram、Discord、Slack、WeChat、WeCom、DingTalk）で以下のコマンドを入力すると、アシスタントをすばやく操作できます。**システム設定 → ショートカットコマンド** で有効・無効の切り替えと一覧の確認ができます。

  | コマンド | エイリアス | 適用範囲 | 説明 |
  | --- | --- | --- | --- |
  | `/help` | `/帮助` `/?` | チャット + IM | 利用可能なすべてのショートカットコマンドを一覧表示 |
  | `/new` | `/reset` `/重启` `/新建` `/restart` | チャット + IM | 現在の会話をクリアして新しい会話を開始 |
  | `/stop` | `/停止` | チャット + IM | 生成中の応答を停止 |
  | `/retry` | `/重试` | チャットのみ | 直前の応答を再生成 |
  | `/model [provider:model]` | `/模型` | チャット + IM | モデルの表示・切り替え（IM では表示のみ） |
  | `/personality [name]` | `/角色` `/persona` | チャット + IM | 人格の表示・切り替え |
  | `/skills` | `/技能` | チャット + IM | 利用可能なスキル一覧を表示 |
  | `/id` | - | IM のみ | 現在のセッション/チャンネルの ChatID を表示 |
  | `/sub` | `/订阅` | IM のみ | 現在のセッションをプロアクティブメッセージ一覧に追加 |
  | `/unsub` | `/取消订阅` | IM のみ | 現在のセッションをプロアクティブメッセージ一覧から削除 |
  | `/plan` * | `/计划` | チャット + IM | プランモード（読み取り専用）に切り替え |
  | `/read` * | `/只读` | チャット + IM | 読み取り専用モードに切り替え（各操作で確認） |
  | `/edit` * | `/编辑` | チャット + IM | 編集許可モードに切り替え |
  | `/yolo` * | - | チャット + IM | Yolo モード（完全自律）に切り替え |
  | `/cowork` * | `/协作` | チャット + IM | 協働モード（サブエージェント連携）に切り替え |
  | `/goal` * | `/目标` | チャット + IM | ゴールモード（完了まで自律実行）に切り替え |
  | `/<skill-name>` * | - | チャット + IM | 指定したスキルを注入 |
  | `#<text>` * | - | チャット + IM | ワークスペースメモリを保存 |
  | `@<path>` * | - | チャットのみ | ワークスペースのファイルを参照 |

  > `*` PC のコマンドライン制御を有効にする必要があります。スキル注入・`#` メモリ・`@` ファイル参照にはワークスペースの設定も必要です。
